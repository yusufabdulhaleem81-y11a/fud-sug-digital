...
// import { Button, Card, Field, inputClass, useToast } from '../../components/ui';
...
import { homeForRole } from '../../lib/routing';
import { useInstitution } from '../../hooks/useInstitution';
import { Button, Card, Field, Icon, inputClass, useToast } from '../../components/ui';

const PORTALS = [
  { icon: 'user', name: 'Student Portal', note: 'Reports, complaints, suggestions & tracking' },
  { icon: 'users', name: 'EXCO Portal', note: 'Casework, tasks, monthly reports' },
  { icon: 'shield', name: 'President & VP', note: 'Oversight, reviews & directives' },
  { icon: 'shield-c', name: 'Administrator', note: 'Administrations, officers & settings' },
];
...
export default function Login() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      if (mode === 'signin') {
        const profile = await signIn(email.trim(), password);
        toast(`Welcome, ${profile.full_name.split(' ')[0]} — opening your ${profile.role} portal`);
        navigate(homeForRole(profile.role), { replace: true });
      } else {
        await registerStudent(email.trim(), password, fullName.trim());
        setNotice('Account created. Check your email to confirm, then sign in.');
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg)', padding: '40px 16px' }}>
      <div className="mx-auto w-full" style={{ maxWidth: 920 }}>
        <div className="mb-6 text-center">
          <img src={settings.logo_path} alt={`${settings.short_name} logo`} className="mx-auto mb-3 h-16 w-16 object-contain" />
          <h1 className="serif text-3xl font-bold">SUG Digital</h1>
          <p className="text-sm text-stone-500">{settings.institution_name}</p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card className="p-6">
            ...
            <p className="mt-4 rounded-lg p-3 text-center text-xs" style={{ background: 'var(--green-soft)', color: 'var(--green-2)', fontWeight: 600 }}>
              All roles sign in here — students, EXCO, VP, President & administrators.
              The system takes you to your portal automatically.
            </p>
          </Card>

          <div>
            <p className="overline mb-3">One platform · Five portals</p>
            <div className="grid gap-3">
              {PORTALS.map((p) => (
                <Card key={p.name} className="fx p-4" >
                  <span style={{ width: 42, height: 42, borderRadius: 11, background: 'var(--green-soft)', color: 'var(--green)', display: 'grid', placeItems: 'center', flex: 'none' }}>
                    <Icon name={p.icon} className="ic-lg" />
                  </span>
                  <div>
